...
def get_settings():
    conn = get_db()
    rows = conn.execute(
        'SELECT key,value FROM settings WHERE user_id=?', (current_user.id,)).fetchall()
    conn.close()
...
def update_note(nid):
    conn = get_db()
    conn.execute('UPDATE sticky_notes SET content=? WHERE id=? AND user_id=?',
                 (request.json['content'], nid, current_user.id))
    conn.commit()
    conn.close()
    return jsonify({'ok': True})
...
def delete_note(nid):
    conn = get_db()
    conn.execute('DELETE FROM sticky_notes WHERE id=? AND user_id=?',
                 (nid, current_user.id))
    conn.commit()
    conn.close()
    return jsonify({'ok': True})
...
def add_todo():
    conn = get_db()
    cur = conn.execute('INSERT INTO todos (user_id,content,done) VALUES (?,?,0)',
                       (current_user.id, request.json['content']))
    conn.commit()
    tid = cur.lastrowid
    conn.close()
...
def update_todo(tid):
    d = request.json
    conn = get_db()
    conn.execute('UPDATE todos SET content=?,done=? WHERE id=? AND user_id=?',
                 (d['content'], d.get('done', 0), tid, current_user.id))
    conn.commit()
    conn.close()
    return jsonify({'ok': True})
...
def delete_weekly(wid):
    conn = get_db()
    conn.execute('DELETE FROM weekly_tasks WHERE id=? AND user_id=?',
                 (wid, current_user.id))
    conn.commit()
    conn.close()
    return jsonify({'ok': True})
